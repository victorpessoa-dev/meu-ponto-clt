...
async function loadDataFor(profile) {
  ...
  state.records = recordsRows.map(toRecord)
  state.justifications = justificationRows.map(toJustification)
}
...
export async function requestPasswordReset(email) {
  ...
  if (error) return { error: error.message || "Nao foi possivel enviar a redefinicao de senha." }
  return { ok: true }
}
...
export async function updateAccountPassword(password) {
  ...
  if (error) return { error: error.message || "Nao foi possivel atualizar a senha." }
...
export async function register(data) {
  ...
  const { data: authData, error } = await supabase.auth.signUp({
    ...
    options: {
      emailRedirectTo: typeof window !== "undefined" ? `${window.location.origin}/confirmar-email` : undefined,
      data: {
        name: data.name,
  ...
  if (authData.user && Array.isArray(authData.user.identities) && authData.user.identities.length === 0) {
    return { error: "Este e-mail ja esta cadastrado. Entre pelo login ou use outro e-mail." }
  }
...
export async function createUser(data) {
  ...
  const row = {
    ...
    punch_fields: normalizePunchFields(data.punchFields),
    clock_offset_minutes: 0,
    clock_offset_seconds: 0,
  }
