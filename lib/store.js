...
export async function verifySignupCode(email, code) {
  ...
  if (!/^\d{6}$/.test(token)) {
    return { error: "Informe o codigo de 6 digitos enviado por e-mail." }
  }
  ...
  if (error) return { error: "Codigo invalido ou expirado. Confira o e-mail e tente novamente." }
...
export async function resendSignupCode(email) {
  ...
  if (error) return { error: "Nao foi possivel reenviar o codigo agora." }
  return { ok: true }
}
